...
function Producer(conf, topicConf) {
  ...
  if (dr_msg_cb || dr_cb) {
    this._deliveryReportHandler = function(err, report) {
      if (err) {
        err = LibrdKafkaError.create(err);
      }
      this.emit('delivery-report', err, report);
    }.bind(this);
    this._deliveryReportHandler.dr_msg_cb = !!dr_msg_cb;
...
/**
 * Save the base connect method here so we can overwrite it and handle delivery reports
 */
Producer.prototype._connect = Producer.prototype.connect;

Producer.prototype.connect = function() {
  if (this._deliveryReportHandler) {
    this._client.onDeliveryReport(this._deliveryReportHandler, this._deliveryReportHandler.dr_msg_cb);
  }
...
Producer.prototype.disconnect = function(timeout, cb) {
  ...
  this.flush(timeoutInterval, function() {
    self._disconnect(function() {
      if (this._deliveryReportHandler) {
        this._client.removeDeliveryReport(this._deliveryReportHandler);
      }
